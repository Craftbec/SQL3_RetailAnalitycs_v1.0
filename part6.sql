...
ttt AS (
SELECT res.cos_id, res.g_id, sku_id, s_name, dis, SKU_Retail_Price,  max_mar,
...
COUN AS (
SELECT Cards.customer_id, ProductGrid.group_id, ProductGrid.sku_id
FROM Cards
JOIN Transactions ON Cards.customer_card_id = Transactions.customer_card_id
...
ok AS (
SELECT ttt.cos_id, s_name as nam, ttt.g_id, sku_id, dis,  max_mar*margin_share/sku_retail_price AS ert
FROM ttt
...
SELECT * FROM CrossSelling(1, 100, 100, 1, 40);
SELECT * FROM CrossSelling(2, 99, 100, 5, 100);
SELECT * FROM CrossSelling(3, 100, 100, 1, 40) WHERE offer_discount_depth >= 10;
SELECT * FROM CrossSelling(4, 100, 100, 1, 40) WHERE customer_id >= 10;
SELECT * FROM CrossSelling(5, 100, 100, 1, 40) WHERE sku_name = 'Чипсы' OR sku_name = 'Колбаса';